...
void draw_segmentation(const Math3D::Tensor<uint>& segmentation, Math3D::Tensor<float>& image, uint t,
                       float r, float g, float b) {
  ...
  for (uint y=0; y < yDim; y++) {
    for (uint x=0; x < xDim; x++) {
      ...
      uint cur_seg = segmentation(x,y,t);
      
      bool edge = false;
      if (x > 0 && segmentation(x-1,y,t) != cur_seg)
        edge = true;
      if (x+1 < xDim  && segmentation(x+1,y,t) != cur_seg)
        edge = true;
      
      if (y > 0 && segmentation(x,y-1,t) != cur_seg)
        edge = true;
      if (y+1 < yDim && segmentation(x,y+1,t) != cur_seg)
        edge = true;
